...
- [ ] 悬浮窗：继续实测桌面版打开、隐藏、置顶、深色背景和右键菜单，确认不再白底、不再点了没反应。
- [ ] 退出清理：实测主窗口关闭、托盘退出、后台 Flask 线程、托盘线程和隐藏悬浮窗是否会留下进程。
- [ ] 设置向导：继续把设置页改成新手可一步步完成的向导，支持以后回来修改，不强迫重跑完整流程。本轮已扩展为 8 步入口，并加入供应商、多模型、路由、媒体兜底、用量/额度/余额来源规则说明；后续还要接入缺项校验和一键填默认值。
- [ ] 中文/英文双语：去掉主界面里过重的技术文案，中文界面补齐汉化，英文保持同等完整。本轮已清理审批、设置向导、供应商兼容说明和诊断区部分英文残留。
