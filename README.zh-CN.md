...
| 统一模型目录 | 预览并过滤 `qwen/qwen3-coder-plus` 这类 Codex 可见模型 ID，支持 provider、capability、context window、cost hint、currency、visibility、focus 和 AMR group。 |
| 自适应模型轮转 | 编辑本地 rotation group、candidate priority、capability、context 限制、provider/model ID，并运行只读 route preview。 |
| 本地代理 | 独立 localhost proxy，支持端口占用自动退避、provider network policy、timeout/retry policy、Auto Approval broker 设置、路由诊断、metadata-only request log 和 OpenAI-compatible 路由。 |
...
- **Adaptive Model Rotation**：rotation group 与 candidate 编辑器，附带基于已保存状态的 route preview。
