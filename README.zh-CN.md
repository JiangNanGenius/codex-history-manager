...
| 本地代理 | 独立 localhost proxy，支持端口占用自动退避、timeout/retry policy、Auto Approval broker 设置、路由诊断、metadata-only request log 和 OpenAI-compatible 路由。 |
...
- **Providers**：preset-first 配置、分区测试、status strip、自定义 `User-Agent`、Auto Approval、媒体模式控制、目录可见性和只读 Route Simulator。
...
实现决策优先参考以下官方源码、官方文档和对照项目：

- OpenAI Codex 源码：[openai/codex](https://github.com/openai/codex)，重点包括 `codex-rs/codex-api/src/endpoint/responses.rs`、`codex-rs/codex-api/src/sse/responses.rs`、`codex-rs/codex-api/src/endpoint/images.rs`、`codex-rs/protocol/src/models.rs`、`codex-rs/app-server/src/bespoke_event_handling.rs` 和 `codex-rs/app-server-protocol/src/protocol/v2/*`。
- OpenAI API 文档：[Responses](https://platform.openai.com/docs/api-reference/responses)、[Chat Completions](https://platform.openai.com/docs/api-reference/chat) 和 [Images](https://platform.openai.com/docs/api-reference/images)。
