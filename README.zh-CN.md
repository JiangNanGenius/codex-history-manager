...
- **Providers**：preset-first 配置、分区测试、status strip、自定义 `User-Agent`、Auto Approval、媒体模式控制、目录可见性、已选模型加入 AMR 和只读 Route Simulator。
- **Unified Model Catalog**：在写入 Codex 之前过滤预览最终模型目录。
- **Adaptive Model Rotation**：rotation group 与 candidate 编辑器，支持已选模型导入和基于已保存状态的 route preview。
