...
如果需要 Code++ 类似的“混入 API key”，可以直接使用 `Codex Native + API Key Mix-in` 预设：Codex 继续保留官方 OAuth 登录态，本地代理负责把 provider API key 请求转发出去。图片生成走 OpenAI-compatible 媒体路径（`/v1/images/*`），因此 provider 需要标记为图片 provider，并把 Media Mode 设为 `OpenAI-compatible` 或 `Adapter required`；媒体 profile 默认项和专用图片/视频 API 格式会同步反映到目录和 AMR 能力。
